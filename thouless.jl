...
e = plot_isoenergies(Heff(h0=0.0, Ω=0.0, I0=1.0, M=2, λL=10, χL=pi, λS=5, χS=0.0, s=2))
...
"Plot `N_bands` energy bands of Hamiltonian (S32)."
function plot_bands(; N_bands, M, λL, λS)
    N_j = 4N_bands # number of indices 𝑗 to take for the Hamiltonian (its size will be (2N_j+1)×(2N_j+1))
    ...
    N_ϕ = 40 # number of adiabatic phases to sweep over
    ...
    bands = Matrix{Float64}(undef, 2N_bands, N_ϕ)

    phases = range(0, 2π, length=N_ϕ)
    for k in [0, 1]
        H[band(0)] .= [(2j + k)^2 / M for j = -N_j:N_j]
        a = k*N_bands + 1 # controls where to place the eigenvalues depedning on `k`; see description of `bands`
        b = a+N_bands - 1 # controls where to place the eigenvalues depedning on `k`; see description of `bands`
        for (i, ϕ) in enumerate(phases)
            H[band(-1)] .= fill(λL*cis(-ϕ), 2N_j)
            H[band(1)] .= fill(λL*cis(ϕ), 2N_j)
            vals, _, _ = eigsolve(H, N_bands, :SR)
            bands[a:b, i] .= vals[1:N_bands] ./ (4 .* π) # we do not include the factor 4π in the diagonalisation problem and restore it here
        end
    end
    fig = plot()
    for i in 1:N_bands
...
# M = 0.4; λL = 10; λS = 7;
M = 0.04; λL = 50; λS = 50;
f = plot_bands(; N_bands=4, M, λL, λS)
