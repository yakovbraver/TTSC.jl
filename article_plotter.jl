...
plot!(θ ./ π, 4abs2.(w[:, 1, i_ϕ]) .+ h.w.E[1, i_ϕ], c=YELLOW, label=L"|w_1|^2")
plot!(θ ./ π, 4abs2.(w[:, 2, i_ϕ]) .+ h.w.E[2, i_ϕ], c=RED, label=L"|w_2|^2", ylims=(-5610, -5575))
...
fig4 = heatmap(θ ./ π, φₜ ./ π, abs2.(w[:, 1, :])', xformatter=_->"", ylabel=L"\varphi_t/\pi", title="x", cbartitle=L"|w_1(\Theta)|^2", c=CMAP, rightmargin=-10mm)
...
fig5 = heatmap(θ ./ π, φₜ ./ π, abs2.(w[:, 2, :])', xlabel=L"\Theta/\pi", ylabel=L"\varphi_t/\pi", cbartitle=L"|w_2(\Theta)|^2", c=CMAP, rightmargin=-10mm)
